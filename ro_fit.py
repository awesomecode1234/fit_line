...
class RO_Fit_Dialog(tk.Toplevel):
    def __init__(self, parent, variable):
        ...
        self.setup_ui()
        self.result = None
        self.Yield_Strength = variable[0]
        self.Elastic_Modulus = variable[1]
        self.m_nys = float(self.Yield_Strength)
        self.m_nem = float(self.Elastic_Modulus)
        # Make modal
        self.grab_set()
    ...
    def setup_ui(self):
        ...
        self.table.heading("Delta_a", text="Strain", anchor="w")
        self.table.heading("J", text="Stress(ksi)", anchor="w")
        ...
        self.fit_button = ttk.Button(button_frame, text="       Fit R-O       ", command=self.fit_curve, state=tk.DISABLED)
    ...
    def fit_curve(self):
        try:
            ...
            def ye_curve(x, a, b):
                return x/self.m_nem+a*(self.m_nys/self.m_nem)*np.power(x/self.m_nys,b)
            
            popt, _ = curve_fit(ye_curve, j1, da1)
            C, m = popt
            self.result = (C, m)
            self.result_label.config(text=f"Inputs: Yield Streingth = {self.Yield_Strength}ksi   Elastic Modulus = {self.Elastic_Modulus}ksi\nOutput: a= {C:.5f}   n = {m:.2f}")
            self.plot_data_with_fit(j,da, ye_curve, popt)
            self.fit_params = (C, m)
            self.report_button.config(state=tk.NORMAL)
        except Exception as e:
            messagebox.showerror("Invalid Data", f"Error: {str(e)}")

    def plot_data_with_fit(self, da, j, ye_curve, popt):
        self.plot_axes.clear()
        self.plot_axes.plot( j,da, 'bo', markersize=8, markeredgewidth=2, markeredgecolor='black', label = "Data")
        da_fine = np.linspace(da.min(), da.max(), 500)
        self.plot_axes.plot(ye_curve(da_fine, *popt),da_fine, 'r-', linewidth=2, label = "Fit")
        self.plot_axes.set_xlabel("Strain", fontsize=12)
        self.plot_axes.set_ylabel("Stress (ksi)", fontsize=12)
        self.plot_axes.grid(True, linestyle='--', linewidth=0.5, color='gray')
    ...
    def generate_report(self):
        file_name = filedialog.asksaveasfilename(defaultextension=".pdf",
                                                 ...
                                                 initialfile="report")
        if file_name:
            plot_image_file = "ro_curve_plot.png"
            self.figure.savefig(plot_image_file, bbox_inches="tight", dpi=300)
            pdf = FPDF()
            pdf.add_page()
            pdf.set_font("Arial", size=12)
            pdf.cell(0, 10, "ROFit V1.0", ln=True, align="C")
            pdf.ln(10)
